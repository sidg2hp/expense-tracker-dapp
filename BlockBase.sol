...
contract ExpenseTracker {
     ...
     * @param _newName The new name for the person
     */
    function updatePersonName(string memory _newName) public {
        // Ensure the sender is registered
        require(people[msg.sender].walletAddress != address(0), "Person not registered");
    ...
    function isRegistered(address _addr) public view returns (bool) {
        // Check if the walletAddress stored for this _addr is the zero address
        // If it's not zero, the person exists and is registered.
