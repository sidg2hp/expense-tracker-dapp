...
function App() {
  ...
  const contractAddress = "0x4a8518Bd42358e7c65EB3d4b921c9F1b8e6CfE9B"; // Paste the address recieved from Remix IDE here
  const [addressToCheck, setAddressToCheck] = useState(''); // State for address input
  const [checkResult, setCheckResult] = useState(''); // State for registration check result
  ...
  const checkIfRegistered = async () => {
    ...
    if (!ethers.utils.isAddress(addressToCheck)) {
      setCheckResult('Invalid Ethereum address format.');
      return;
    }

    try {
      const registered = await contract.isRegistered(addressToCheck);
      setCheckResult(`Address ${addressToCheck.substring(0,6)}... is ${registered ? 'registered' : 'not registered'}.`);
    } catch (error) {
      console.error("Error checking registration:", error);
      setCheckResult('Error checking registration. See console.');
    }
  };
      ...
      <header className="App-header">
        ...
        : (
          <div>
            ...
            <div style={{ margin: '20px 0', padding: '10px', border: '1px solid lightgray' }}>
